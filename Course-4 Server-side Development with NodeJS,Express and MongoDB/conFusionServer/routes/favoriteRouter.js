...
.post(cors.corsWithOptions,authenticate.verifyUser, (req, res, next) => {
    Favorites.findOne({ user: req.user._id })
    .then((favorite) => {
        if (favorite != null && favorite.length != 0 && favorite.dishes.indexOf(req.body[0]._id) === -1) {
...
.post(cors.corsWithOptions,authenticate.verifyUser, (req, res, next) => {
    Favorites.findOne({ user: req.user._id })
    .then((favorite) => {
    if (favorite != null && favorite.length != 0 && favorite.dishes.indexOf(req.params.dishId) === -1 ) {
